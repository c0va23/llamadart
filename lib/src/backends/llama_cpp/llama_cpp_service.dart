...
_libcFdopen = _processLibrary
    .lookupFunction<
      Pointer<FILE> Function(Int32, Pointer<Char>),
      Pointer<FILE> Function(int, Pointer<Char>)
    >('fdopen');
...
class LlamaCppService {
  ...
  int loadModelFromFd(int fileDescriptor, ModelParams modelParams) {
    return _loadModelWithParams(
      ...
      // mmap keeps the mapping valid after the FILE* closes, so the model is
      // demand-paged exactly as for the path-based load.
  ...
  /// Runs [action] with a `FILE*` wrapping a dup of [fileDescriptor], closing
  /// the dup'd stream afterwards. We [dup] so our own `fclose` never disturbs
  /// the caller's fd; the caller retains ownership of [fileDescriptor].
  T _withFilePointerFromFd<T>(
    int fileDescriptor,
    T Function(Pointer<FILE> filePtr) action,
  ) {

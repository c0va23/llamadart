...
class LlamaCppService {
  ...
  int createMultimodalContextFromFd(int modelHandle, int fileDescriptor) {
    ...
    try {
      final ctxParams = _mtmdContextParamsDefault();
      ctxParams.use_gpu = _modelToMtmdUseGpu[modelHandle] ?? true;
      mmCtx = _mtmdInitFromFilePtr(filePtr, model.pointer, ctxParams);
    } finally {
      // Closes duppedFd; the projector is fully read by init, so nothing needs
      // the fd afterwards.
      _libcFclose(filePtr);
    }

    if (mmCtx == nullptr) {
      throw Exception("Failed to load multimodal projector from fd");
    }
